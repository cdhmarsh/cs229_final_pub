...
if __name__ == "__main__":
    ...
    # Create datasets for experiment
    ...
    combined_soft_dataset = ConcatDataset([augmented_soft_dataset, augmented_soft_dataset])
    ...
    for model_name in models_to_test:
        ...
        # Experiment 1: Augmenting CIFAR-10 with soft labels from CIFAR-10H
        print(
            f"\nExperiment 1: Augmenting CIFAR-10 with soft labels from CIFAR-10H ({len(augment_dataset)} augment samples)"
        )
        ...
        if model_name in ["resnet34", "vgg16"]:
            model, metrics = train_model(
                ...
                num_epochs=30,
                device=device,
                model_name=f"{model_name}_experiment1",
            )
            # Evaluate last model
            ...
            best_model.load_state_dict(torch.load(f"models/{model_name}_experiment1.pth", weights_only=True))
        ...
        experiment1_metrics[model_name] = metrics
        save_metrics(metrics, f"{model_name}_experiment1")
